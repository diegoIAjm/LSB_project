...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--max-frames', type=int, default=30, help='Máximo de frames a procesar')
        parser.add_argument('--confianza', type=float, default=0.3, help='Umbral de confianza para detección (0.0-1.0, menor=mas detecciones)')
        ...
        parser.add_argument('--resize', type=float, default=0.5, help='Factor de redimensionamiento (0-1, ej: 0.5 para reducir a la mitad)')
        parser.add_argument('--detectar-pose', action='store_true', help='Detectar también landmarks de pose (hombros, codos, muñecas)')
    ...
    def handle(self, *args, **options):
        ...
        if not HAS_TASKS:
            ...
            self.stdout.write('   Intenta: pip install --upgrade mediapipe')
            return
        ...
        self.stdout.write(f'   Resolución original: {original_width}x{original_height}')
        self.stdout.write(f'   Resolución procesamiento: {new_width}x{new_height}')
        ...
        if detectar_pose:
            ...
            if not os.path.exists(pose_model_path):
                self.stdout.write(f'   ⚠️ Modelo de pose no encontrado, descargando...')
        ...
        while cap.isOpened() and processed_frames < max_frames:
            ...
            # Inicializar arrays
            pose_landmarks = [0.0] * 99  # 33 landmarks * 3
            right_hand = [0.0] * 63
            left_hand = [0.0] * 63
            ...
            if hand_results.hand_landmarks and len(hand_results.hand_landmarks) > 0:
                ...
                for hand_idx, hand_landmarks in enumerate(hand_results.hand_landmarks):
                    if hand_idx < len(hand_results.handedness) and len(hand_results.handedness[hand_idx]) > 0:
            ...
            if pose_results and pose_results.pose_landmarks and len(pose_results.pose_landmarks) > 0:
                ...
                # Extraer todos los landmarks de pose (33 puntos)
                ...
                if debug_video and 'frame_annotated' in locals():
                    # Puntos importantes: hombros(11,12), codos(13,14), muñecas(15,16)
            ...
            status = f'\r   Frame {processed_frames}/{max_frames} - R:{1 if has_right else 0} L:{1 if has_left else 0} Pose:{1 if has_pose else 0} | M:{detecciones_manos} P:{detecciones_pose}'
        ...
        if debug_video:
            self.stdout.write(self.style.SUCCESS(f'🎥 Video debug guardado en: {debug_output_path}'))